...
Boots_Phyglm_LOO_CV_Cont <- function(Data, B){
  ...
  indseulOTU <- which(apply(Data$X_AR, 2, function(x)sum(x!=0))==1) #Chercher les OTUs qui sont que chez un seul patient
  ...
  for(j in indseulOTU){
    k=k+1
    indseul[k] <- which(Data$X_AR[,j]!=0) #On repère les individus
  }
  ...
  for(b in 1:B){
    ...
    bootid <- sample(seq(1:dim(Data$X_AR)[1])[-indseul], dim(Data$X_AR)[1] - length(indseul), replace = TRUE)
    fit_PhyLasso_Boot <- phyglm_LOO_CV_Cont(x = Data$X_AR[bootid,], y = Data$y[bootid,], taxonomy = Data$Taxon,
                                        family = "gaussian")
